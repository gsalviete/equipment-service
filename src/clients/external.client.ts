...
export class ExternalClient {
  ...
  constructor(
    ...
    this.baseURL = this.configService.get<string>('EXTERNAL_SERVICE_URL') ||
      'https://external-service-api.onrender.com';
    this.logger.log(`External Service URL: ${this.baseURL}`);
  }

  async sendEmail(email: string, assunto: string, mensagem: string): Promise<any> {
    try {
      ...
      this.logger.error(`Failed to send email to ${email}: ${axiosError.message}`);
